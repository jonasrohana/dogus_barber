...
class _EditProfileState extends State<EditProfile> {
  ...
  Future<void> editProfile() async {
    ...
    String imageUrl = "";
    if(image != null) {
      if(image!.isFile && image!.croppedFile != null) {
      ...
      else if(!image!.isFile && image!.url != null) {
        imageUrl = image!.url!;
      }
    }
    ...
    await showDialog(
      ...
      builder: (BuildContext context) =>
        AnimationDialog(text: "general.edited".tr(), isSuccess: true)
    );
    if(!mounted)return;
  ...
  void showImageActionSheet() {
    ...
    showModalBottomSheet(
      ...
      builder: (BuildContext context) => ActionSheet(
        actions: [
          ActionSheetAction(
            name: "general.gallery".tr(),
            onPressed: () {
              pickImage();
            }, icon: Ionicons.add,
          ),
          ActionSheetAction(
              name: "general.deletePic".tr(),
              destructive: true,
  ...
  void prefill() {
    ...
    if (user != null) {
      if(user.imageUrl.isNotEmpty) {
        image = DynamicImage(isFile: false, url: user.imageUrl);
      }
      phoneNr.text = user.phoneNr;
      name.text = user.name;
    }
  }
  ...
  Widget get imagePicker {
    ...
    if(image == null) {
      child = Container(
        ...
        child: Icon(Icons.image_outlined, size: 50, color: Colors.black.withValues(alpha: 0.6)),
      );
    }
